...
async function sendWelcomeEmail(env: Env, data: { firstName: string; workEmail: string; hospitalName: string }) {
  if (!env.MAILTRAP_TOKEN) return;
  const emailHtml = `
...
async function sendDiscordNotification(webhookUrl: string | undefined, data: any) {
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // Asynchronous non-blocking background notifications using context.waitUntil
    const backgroundTasks: Promise<any>[] = [];
    
    if (context.env.MAILTRAP_TOKEN) {
      backgroundTasks.push(
        sendWelcomeEmail(context.env, data).catch((err) =>
          console.error('Mailtrap background error:', err)
        )
      );
    }
    ...
    if (context.env.DISCORD_WEBHOOK_URL) {
      backgroundTasks.push(
        sendDiscordNotification(context.env.DISCORD_WEBHOOK_URL, data).catch((err) =>
          console.error('Discord webhook background error:', err)
        )
      );
    }

    if (backgroundTasks.length > 0 && typeof (context as any).waitUntil === 'function') {
      (context as any).waitUntil(Promise.allSettled(backgroundTasks));
    }
