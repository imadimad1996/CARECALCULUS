      ...
      >
        ...
        <div className="h-full flex flex-col pt-5 md:pt-8 pb-4 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700 scrollbar-track-transparent">
          ...
          <div className="px-5 mb-5 hidden md:flex items-center justify-between gap-2 min-w-0">
            ...
            {/* Globe Language Toggle — click-based */}
            <div className="relative shrink-0">
              <button
                type="button"
                onClick={() => setIsLangOpen(o => !o)}
                onBlur={(e) => { if (!e.currentTarget.parentElement?.contains(e.relatedTarget as Node)) setIsLangOpen(false); }}
              ...
              >
                ...
                <ChevronDown className={`w-3 h-3 text-slate-400 shrink-0 transition-transform duration-200 ${isLangOpen ? 'rotate-180' : ''}`} />
              </button>

              {/* Dropdown Options — shown on click only */}
              {isLangOpen && (
                <div
                  role="listbox"
                  aria-label="Select language"
                  className="absolute right-0 top-full mt-1.5 w-36 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-1.5 z-50 animate-fade-in"
                >
                  <a
