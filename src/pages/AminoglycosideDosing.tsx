      ...
      <div className="w-full max-w-full max-w-3xl relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
          ...
          <div>
            ...
            <input type="number" inputMode="decimal" value={age} onChange={(e) => setAge(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-3.5 min-h-[48px] bg-slate-50 border border-slate-200 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-500/10 rounded-2xl text-lg font-semibold transition" placeholder="e.g. 45" />
          </div>
          ...
          <div>
            ...
            <input type="number" inputMode="decimal" value={weight} onChange={(e) => setWeight(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-3.5 min-h-[48px] bg-slate-50 border border-slate-200 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-500/10 rounded-2xl text-lg font-semibold transition" placeholder="e.g. 70" />
          </div>

          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">{dict.height}</label>
            <input type="number" inputMode="decimal" value={height} onChange={(e) => setHeight(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-3.5 min-h-[48px] bg-slate-50 border border-slate-200 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-500/10 rounded-2xl text-lg font-semibold transition" placeholder="e.g. 175" />
          </div>

          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">{isSI ? dict.scrSI : dict.scr}</label>
            <input type="number" inputMode="decimal" step={isSI ? "1" : "0.1"} value={scr} onChange={(e) => setScr(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-3.5 min-h-[48px] bg-slate-50 border border-slate-200 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-500/10 rounded-2xl text-lg font-semibold transition" placeholder={isSI ? "e.g. 88" : "e.g. 1.0"} />
          </div>
        </div>
