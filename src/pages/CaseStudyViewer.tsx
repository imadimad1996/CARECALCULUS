    ...
    <div className="max-w-3xl mx-auto py-6 px-4" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="mb-8 text-center sm:text-left">
        <span className="text-[10px] font-mono font-black text-teal-600 uppercase tracking-widest">
          {lang === 'fr' ? 'PRATIQUE CLINIQUE INTERACTIVE' : isRtl ? 'تدريب الحالات السريرية التفاعلية' : 'CLINICAL SIMULATION'}
        </span>
        <h1 className="text-3xl font-black text-slate-900 mt-1">
          {lang === 'fr' ? caseData.titleFr : isRtl ? caseData.titleAr : caseData.titleEn}
        </h1>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-200/85 p-6 md:p-8 space-y-6 shadow-xs">
        ...
        {activeStep.vitals && (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 bg-gray-50 p-4 rounded-2xl border border-gray-150">
            <div className="text-center">
              <span className="text-[9px] font-mono text-gray-400 font-bold uppercase block">Heart Rate</span>
            ...
            <div className="text-center border-l sm:border-r border-gray-200/60">
              <span className="text-[9px] font-mono text-gray-400 font-bold uppercase block">Blood Pressure</span>
            ...
            <div className="text-center border-l border-gray-200/60">
              <span className="text-[9px] font-mono text-gray-400 font-bold uppercase block">Temperature</span>
              <span className="text-sm font-black text-orange-500">{activeStep.vitals.temp} °C</span>
            </div>
            <div className="text-center border-l border-gray-200/60">
              <span className="text-[9px] font-mono text-gray-400 font-bold uppercase block">Resp Rate</span>
              <span className="text-sm font-black text-blue-500">{activeStep.vitals.rr} /min</span>
            </div>
            <div className="text-center border-l border-gray-200/60">
              <span className="text-[9px] font-mono text-gray-400 font-bold uppercase block">SpO2</span>
        ...
        {activeStep.question && (
          <div className="border-t border-gray-100 pt-6 space-y-4">
            ...
            <div className="space-y-2">
              {activeStep.question.options.map((opt, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 text-left border rounded-xl text-xs font-semibold flex items-center justify-between transition-all duration-200 cursor-pointer ${btnStyle}`}
            ...
            {!isAnswered ? (
              <button
                ...
                className="w-full py-3 bg-blue-650 hover:bg-blue-700 disabled:opacity-50 text-white font-bold rounded-xl text-xs tracking-wide transition active:scale-95"
              >
                SUBMIT DECISION
              </button>
            ) : (
              <div className="bg-blue-50/50 border border-blue-100/80 p-4 rounded-2xl space-y-2 animate-fade-in text-xs leading-relaxed text-slate-700">
                <span className="font-bold text-blue-700 uppercase tracking-wide block">Clinical Explanation</span>
                <p>{activeStep.question.rationale}</p>
                <div className="border-t border-blue-100/60 pt-2 mt-2 text-[10px] text-gray-400 font-mono">
                  Reference: Surviving Sepsis Campaign Guidelines 2021 / JAMA.
                </div>
              </div>
            )}
          </div>
        ...
        {isAnswered && currentStep < caseData.steps.length - 1 && (
          <button
            onClick={handleNext}
            className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 transition active:scale-95 mt-4"
          >
            <span>PROCEED TO NEXT CLINICAL STEP</span>
