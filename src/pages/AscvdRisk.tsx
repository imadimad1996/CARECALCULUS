      ...
      <div className="w-full max-w-full max-w-3xl relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
          <div>
            ...
            <input type="number" inputMode="decimal" value={age} onChange={(e) => setAge(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-3.5 min-h-[48px] bg-slate-50 border border-slate-200 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 rounded-2xl text-lg font-semibold transition" placeholder="e.g. 55" />
          </div>

          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">{dict.sbp}</label>
            <input type="number" inputMode="decimal" value={sbp} onChange={(e) => setSbp(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-3.5 min-h-[48px] bg-slate-50 border border-slate-200 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 rounded-2xl text-lg font-semibold transition" placeholder="e.g. 120" />
          </div>

          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">{isSI ? dict.tcSI : dict.tc}</label>
            <input type="number" inputMode="decimal" step={isSI ? "0.1" : "1"} value={tc} onChange={(e) => setTc(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-3.5 min-h-[48px] bg-slate-50 border border-slate-200 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 rounded-2xl text-lg font-semibold transition" placeholder={isSI ? "e.g. 5.0" : "e.g. 180"} />
          </div>

          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">{isSI ? dict.hdlSI : dict.hdl}</label>
            <input type="number" inputMode="decimal" step={isSI ? "0.1" : "1"} value={hdl} onChange={(e) => setHdl(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-3.5 min-h-[48px] bg-slate-50 border border-slate-200 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 rounded-2xl text-lg font-semibold transition" placeholder={isSI ? "e.g. 1.2" : "e.g. 50"} />
          </div>
