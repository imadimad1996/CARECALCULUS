...
HTML_SPLASH_SCREEN = f"""<!DOCTYPE html><html><head><style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  body {{ width: 2048px; height: 2048px; display: flex; flex-direction: column; align-items: center; justify-content: center; background: radial-gradient(circle at 50% 40%, #152945 0%, #0b1329 70%); font-family: 'Inter', system-ui, -apple-system, sans-serif; overflow: hidden; }}
...
for idx, (filename, width, height, html_content) in enumerate(TASKS):
    html_file = os.path.join(TEMP_DIR, f"finish_{idx}.html")
    raw_png = os.path.join(TEMP_DIR, f"finish_{idx}_raw.png")
    final_png = os.path.join(ASSETS_DIR, filename)

    with open(html_file, "w", encoding="utf-8") as f:
        f.write(html_content)

    user_dir = os.path.join(TEMP_DIR, f"finish_user_{idx}")
    cmd = [
        EDGE_PATH,
        "--headless",
        "--disable-gpu",
    ...
    if os.path.exists(raw_png):
        ...
        print(f"SUCCESS: {filename} ({width}x{height}) -> {os.path.getsize(final_png)} bytes")
    else:
